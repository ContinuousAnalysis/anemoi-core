...
class Config(NodeDict):
    ...
    def add_summary(self, content: str | Iterable[str]) -> None:
        r"""Add some information about the goal of the migration script.

        Parameters
        ----------
        content : str | Iterable[str]
            The summary to add to the config. If it's not a str, the items in the Iterable
            will be joined by a `\n`.
        """
